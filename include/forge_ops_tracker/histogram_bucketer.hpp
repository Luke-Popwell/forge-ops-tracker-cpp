 ...
 *
 * kHistogramBoundariesMs is duplicated on the server side, in app/services/histogram_percentile.rb.
 * Change one, change the other, or a released SDK version and the server it talks to would
 * silently disagree about what each bucket label means.
 */
inline constexpr std::array<unsigned long, 8> kHistogramBoundariesMs = {50, 100, 250, 500, 1000, 2500, 5000, 10000};

/**
 * The label of the smallest boundary duration_ms fits under, or "inf" for anything larger than the
 * largest boundary. A std::string, not a number: this travels as a JSON object key once flushed,
