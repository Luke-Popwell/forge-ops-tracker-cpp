...
void Reporter::report(const std::exception_ptr& exception_ptr, const nlohmann::json& context, const nlohmann::json& user, const nlohmann::json& breadcrumbs) {
    try {
        if (!configuration_.is_enabled()) {
            return;
        }
        nlohmann::json payload = event_builder_.build(exception_ptr, context, user, breadcrumbs);
...
void Reporter::report(const std::exception& exception, const nlohmann::json& context, const nlohmann::json& user, const nlohmann::json& breadcrumbs) {
    ...
    report(current ? current : std::make_exception_ptr(exception), context, user, breadcrumbs);
}
